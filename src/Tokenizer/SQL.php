...
class SQL implements TokenizerInterface
{
    ...
    /*private*/ const _x_keywords_ln = '(?:' . self::_x_keywords_top.'|XOR|AND|OR|ADD)';
                    ...
                    switch ($token['type']) {
                        ...
                        case self::TOKEN_KEY:
                            if (preg_match('~^(' . self::_x_keywords_top . ')~i', $token['value'])
                                ...
                                $indent++;
                                $indents[] = 'key';
                            } elseif (($next = $tokens[$idx + 1] ?? null) && !($next['value'] == ',' || $next['value'] == ')' || $next['value'] == '.' || $next['value'] == ';')) {
                                $_tokens[] = ['type' => self::TOKEN_SPACE, 'value' => ' '];
                            }
                            break;
                        case self::TOKEN_COMMENT:
                            ...
                            switch ($token['value']) {
                                ...
                                case ',':
                                    switch ($indents[count($indents)-1] ?? null){
                                        case 'key':
                                        case 'indent':
                                            if (($next = $tokens[$idx + 1] ?? null) && !($next['type'] == self::TOKEN_INT || $next['type'] == self::TOKEN_STRING || $next['type'] == self::TOKEN_COMMENT)) {
                                                $_tokens[] = ['type' => self::TOKEN_SPACE, 'value' => PHP_EOL];
                                                break;
                                            }
                                        case 'inline':
